...
class ToPoincare(nn.Module):
    r"""
    constraint points into n-dim Poincare ball
    """

    def __init__(self, c, train_c=False, riemannian=True):
        super(ToPoincare, self).__init__()
        self.register_parameter("xp", None)

        if train_c:
            #self.c = nn.Parameter(torch.exp(-torch.abs(torch.Tensor([c,]))))
            self.c = nn.Parameter(torch.abs(torch.Tensor([c, ])))
        else:
            self.c = c
    ...
    def forward(self, x):
        return self.grad_fix(pmath.project(x, c=self.c))
