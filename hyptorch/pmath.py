...
def _dist_trans(x, c):
    #sqrt_c = (torch.cos(x[:,0] * (torch.norm(x, dim=-1) ** (-1))) * c) ** 0.5
    sqrt_c = c ** 0.5
    return (
        x
        * ((sqrt_c * torch.norm(x, dim=-1)) ** (-1)).unsqueeze(1)
        * artanh(sqrt_c * torch.norm(x, dim=-1)).unsqueeze(1)
    )


def dist_trans(x, c=1.0):
    c = torch.as_tensor(c).type_as(x)
    return _dist_trans(x, c)
